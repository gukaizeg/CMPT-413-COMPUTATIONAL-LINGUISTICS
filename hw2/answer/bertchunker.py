...
def introduce_spelling_errors(word, error_rate=0.4):
    """Introduce a simple spelling error into a word."""
    if random.random() < error_rate:
        operation = random.choice(['swap', 'insert', 'delete', 'replace'])
        if operation == 'swap' and len(word) > 1:
            i = random.randint(0, len(word)-2)
            word = word[:i] + word[i+1] + word[i] + word[i+2:]
        elif operation == 'insert':
            i = random.randint(0, len(word)-1)
            word = word[:i] + random.choice('abcdefghijklmnopqrstuvwxyz') + word[i:]
...
def read_conll(handle, input_idx=0, label_idx=2, is_training=False):
    conll_data = []
    contents = re.sub(r'\n\s*\n', r'\n\n', handle.read())
    ...
    for sent_string in contents.split('\n\n'):
        ...
        # Modify this block to introduce spelling errors
        if is_training:
            modified_sentence1 = []
            modified_sentence2 = []
            for word in annotations[input_idx]:
                modified_word1 = introduce_spelling_errors(word)
                modified_word2 = introduce_spelling_errors(word)
                modified_sentence1.append(modified_word1)
                modified_sentence2.append(modified_word2)
            if label_idx < 0:
                conll_data.append(tuple(modified_sentence1))
                conll_data.append(tuple(modified_sentence2))
                conll_data.append(annotations[input_idx])
            else:
                conll_data.append((tuple(modified_sentence1), annotations[label_idx]))
                conll_data.append((tuple(modified_sentence2), annotations[label_idx]))
...
class TransformerModel(nn.Module):
    ...
    def init_model_from_scratch(self, basemodel, tagset_size, lr):
        ...
        #self.crf_layer = CRF(num_tags=tagset_size, batch_first=True)
        ...
        #crf_optimizer = optim.Adam(self.crf_layer.parameters(), lr=5e-4)  # Use the same LR as encoder
    ...
    def forward(self, sentence_input):
        ...
        # tags = ["B-NP","I-NP","O","B-VP","B-PP","I-VP","B-ADVP","B-SBAR","B-ADJP","I-ADJP","B-PRT","I-ADVP","I-PP","I-CONJP","I-SBAR","B-CONJP","B-INTJ","B-LST","I-INTJ","I-UCP","I-PRT","I-LST","B-UCP"]
        # tag_scores = self.crf_layer(tag_space,tags)
        return tag_scores
...
class FinetuneTagger:
    ...
    def load_training_data(self, trainfile, is_training=False):
        if trainfile[-3:] == '.gz':
            with gzip.open(trainfile, 'rt') as f:
                self.training_data = read_conll(f, is_training=is_training)
        else:
    ...
    def train(self):
        self.load_training_data(self.trainfile, is_training=True)
