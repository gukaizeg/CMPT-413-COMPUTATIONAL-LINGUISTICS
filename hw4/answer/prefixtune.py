...
def remove_duplicate(sentence):
    ...
    for s in sentence:
        ...
        if len(words) >=2:
            ...
            if ending not in unique_ending:
                unique_ending.add(ending)
                result.append(s.strip())
        elif s.strip():
            result.append(s.strip())
    return '. '.join(result)+('.' if result else '')
    
def remove_seq(sentence):
        ...
        while change:
            ...
            if len(words) >=n:
                ...
                for i in range(len(words)-n):
                    if remove_punctuation_and_lower(''.join(words[i:i+n]).replace(" ", "")) == remove_punctuation_and_lower(''.join(last_words).replace(" ", "")):
                        del words[i:i+n]
                        change = True
                        break
        n -= 1
...
def cut_sentence(sentence):
    ...
    start = sentence.split()[-1]
    candiate = []
    check = []
    for item in sentence.split(start):
        temp = remove_punctuation_and_lower(item)
        if temp not in check:
            candiate.append(item)
            check.append(temp)
    answer = start.join(candiate)
    if answer[-1] != '.':
        answer += '.'
    return answer

def rate_sentence(sentence,target):
    count = 0
    set = []
    for t in target:
        for i in t.split():
    ...
    for phrase in set:
        ...
        count += sentence_lower.count(phrase_lower)
    return count
...
class TableToText:
    ...
    def train(self):
        ...
        prefix_tuning_config = PrefixTuningConfig(num_virtual_tokens=self.virtualtokens, 
    ...
    def predict(self, model, src, num_sequences=1):
        ...
        with torch.no_grad():
            for itr in range(5):
                ...
                answer = text.lstrip().replace(self.prompt + src, "").replace("\n", " ")
                answer = cut_sentence(answer)
                answer = cut_sentence(answer)
                candidate = []
                for sentence in answer.split('.'):
                    if sentence not in candidate:
                        candidate.append(sentence)
                answer = '.'.join(candidate)
                answer = remove_duplicate(answer)
                answer = remove_seq(answer)
                temp = []
                for s in answer.split('.'):
                    temp.append(remove_seq(s))
                answer = '. '.join(temp)
                if remove_punctuation_and_lower(answer.split()[-1]) in ["the","near","by","called","in"]:
                    answer = ' '.join(answer.split()[:-1])+'.'
                if remove_punctuation_and_lower(answer.split()[-1]) in ["the","near","by","called","in"]:
                    answer = ' '.join(answer.split()[:-1])+'.'
                score = rate_sentence(answer,target)
                if score > highest_score or prediction == None:
                    highest_score = score
                    prediction = answer
            return prediction
