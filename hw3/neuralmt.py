...
class AttentionModule(nn.Module):
    ...
    def calcAlpha(self, decoder_hidden, encoder_out):
        ...
        return alpha  # 注意这里直接返回(batch, seq_len)，适合后续操作

    def forward(self, decoder_hidden, encoder_out):
        # 注意：decoder_hidden的输入形状应该是(batch, dim)
        alpha = self.calcAlpha(decoder_hidden, encoder_out)  # (batch, seq_len)
        alpha = alpha.unsqueeze(1)  # (batch, 1, seq_len) 适用于torch.bmm
        encoder_out = encoder_out.permute(1, 0, 2)  # (batch, seq_len, dim)
        context = torch.bmm(alpha, encoder_out)  # (batch, 1, dim)
        context = context.permute(1, 0, 2)  # (1, batch, dim) 若需要调整回原始维度
        alpha = alpha.squeeze(1)  # 若需要返回alpha，确保其维度正确
